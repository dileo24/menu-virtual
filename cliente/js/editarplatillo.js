...
(function () {
  ...
  function mostrarPlatillo(platilloResult) {
    nombreInput.value = platilloResult.nombre;
    descripcionInput.value = platilloResult.descripcion;
    precioInput.value = platilloResult.precio;
    idInput.value = platilloResult.id;
  }
  ...
  function validarPlatillo(e) {
    ...
    // Para garantizar que la alerta de éxito se muestre, se demora un segundo la ejecución de la función que carga los datos en el json,
    // debido a que al cargar datos en una API, la página se recarga. Además, para que el usuario no interrumpa la carga de datos yéndose a
    // la página 'Menú' o 'Nuevo platillo' (en ese segundo), se elimina el atributo 'href' del botón menú...
